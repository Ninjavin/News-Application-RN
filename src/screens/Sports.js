...
import { updateSports } from "../../redux/newsApiSlice";
...
export default function Sports() {
  ...
  const apiData = useSelector((state) => state.news.sportsArticles);
  ...
  const storeData = async (value) => {
    try {
      ...
      await AsyncStorage.setItem("sports", jsonValue);
    } catch (e) {
      // saving error
      console.log(e);
    }
  ...
  useEffect(() => {
    if (response) {
      // localStorage.setItem("sportsArticles", JSON.stringify(response));
      storeData(response);
      dispatch(updateSports(response));
    }
  }, [response, error]);
  ...
  useEffect(() => {
    async function getData() {
      let data;
      try {
        const jsonValue = await AsyncStorage.getItem("sports");
